...
| ----- | ------------------------------------------------------------------------- | -------------------------------------- | ------------------------- |
| 0     | Workspace, strict TS, lint/format, Postgres, Prisma, shared envelopes, CI | `pnpm verify`, health route, migration | Local complete; DB gate   |
| 1     | Router, SparkDEX adapter, deployment scripts, Foundry tests               | unit/fuzz/invariant tests              | Complete                  |
...
- Automated tests: 125 passed, including 29 Foundry
...
- PostgreSQL integration/migration acceptance: blocked because the local Docker
  engine is unavailable.
- Credentialed Xaman/XRPL/FDC/Coston2 smoke: not run; no keys, hosted callback,
  or deployed router manifest was supplied.
